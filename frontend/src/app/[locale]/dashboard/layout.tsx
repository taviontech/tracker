...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)]">
      {/* Sidebar */}
      <aside className="w-56 shrink-0 border-r border-white/[0.06] pt-8 pb-6 px-3 hidden md:block">
        <div className="mb-6 px-3">
          <p className="text-xs text-slate-600 font-medium uppercase tracking-wider">{t('title')}</p>
          {auth?.memberships?.[0]?.companyName && (
            <p className="text-sm text-slate-400 mt-1 truncate">{auth.memberships[0].companyName}</p>
          )}
        </div>
          ...
          {NAV_KEYS.map(item => {
            ...
            return (
              <Link key={item.key} href={href} className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                isActive ? 'bg-blue-500/20 text-blue-300' : 'text-slate-500 hover:text-slate-300'
              }`}>
                {t(item.tKey as any)}
              </Link>
            );
